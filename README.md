...
```
dbx execute --cluster-name=<name of interactive cluster> --job=cicd_demo1-sample-integration-test
```

For a test on a automated job cluster, use `launch` instead of `execute`:
```
dbx execute --job=cicd_demo1-sample-integration-test
```
...
```
dbx launch --job=cicd_demo1-sample
```
